jQuery(document).ready(function($){
    ...
    $(".bk4").mouseover(function(){
        $(".bk4").hide();
        $(".bk44").show();
    })
    $(".bk44").mouseleave(function(){
        $(".bk4").show();
        $(".bk44").hide();
    })
})
...
!function(n){
    var i=function(i,e){
        function c(){
            r.config.current<d-2&&(r.config.current++,o.stop(!0,!1).animate({
                left:-r.config.w*r.config.current-r.config.w
            },r.config.speed,function(){
                v(r.config.current),r.config.current===d-2&&(o.css("left",-r.config.w),r.config.current=0,v(r.config.current))
            }))}
            function t(){
                clearInterval(w),w=setInterval(c,r.config.intervalTime)
            }
            var r=this;
            this.el=i,this.userConfig=e,this.config={
                w:this.el.width(),current:0,speed:500,intervalTime:5e3
            ...
            g.on("click",function(n){n.preventDefault(),r.config.current<d-2&&(t(),r.config.current++,r.config.current!=d-2&&o.stop(!0,!1).animate({left:-r.config.w*r.config.current-r.config.w},r.config.speed,function(){v(r.config.current)}),r.config.current===d-2&&o.stop(!0,!1).animate({left:-r.config.w*r.config.current-r.config.w},r.config.speed,
                function(){o.css("left",-r.config.w),r.config.current=0,v(r.config.current)}))}),s.on("click",function(n){
                    n.preventDefault(),r.config.current>-1&&(t(),r.config.current--,r.config.current!=-1&&o.stop(!0,!1).animate(
                        {left:-r.config.w*r.config.current-r.config.w},
                        r.config.speed,function(){v(r.config.current)}),
                        r.config.current===-1&&o.stop(!0,!1).animate({left:0},
                            r.config.speed,function(){o.css("left",-r.config.w*(d-2)),
                            r.config.current=d-3,v(r.config.current)}))}),p.on("click",
                            function(i){
                                i.preventDefault(),t();var e=n(this).index();v(e),o.stop(!0,!1).animate({
                                    left:-r.config.w*e-r.config.w},r.config.speed,function(){
                                        r.config.current=e})});
                                        var w=setInterval(c,r.config.intervalTime)};
                                        n.fn.extend({xSlider:function(){new i(n(this))}})}(jQuery);
                                        var config={current:0,speed:500,intervalTime:2e3};$(".slider").xSlider(config);
