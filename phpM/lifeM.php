...
//获得中的记录数
  $sql_total_records="select count(*) from baozhang";
...
//获取到了要访问的页面以及页面大小,下面开始分页
  ...
  $sql="select * from baozhang limit $begin_position,$page_size";
...
<!--查询-->
<div class="sel">
	<form method="post" action="selAction.php?active=7">
		<select name="type">
...
<!--显示-->
<div class="conright">
	<form method="post" action="lifeAction.php?action=delete">
		<table>
			<caption class="m-cap">最低生活保障信息管理</caption>
			<tr>
        ...
        <th>最低保障金额</th>
          ...
          <td><?php echo $row[minmoney] ?></td>
					<td>
						<input type='button' name='update' value='修改' style='cursor: pointer;height: 22px;width: 40px;' onclick="window.location.href='lifeupdate.php?id=<?php echo $row[id] ?>'"/>
					</td>
				</tr>
			<?php } ?>
			<tr>
				<td class="insert" colspan="11">
					<input type="button" name="register" value="插入" class="btn" onclick="window.location.href='lifeInsert.php'">
			...
			<tr>
				<td colspan="11" id="fpage">
					<?php
					    echo '<a href="lifeM.php?page_number=1">首页</a>&nbsp;&nbsp;';
					    echo '<a href="lifeM.php?page_number='.($current_page_number-1).'">上一页</a>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;';
					    echo "$current_page_number/$total_pages&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;";
					    echo '<a href="lifeM.php?page_number='.($current_page_number+1).'">下一页</a>&nbsp;&nbsp;';
					    echo '<a href="lifeM.php?page_number='.($total_pages).'">尾页</a>&nbsp;&nbsp;';
